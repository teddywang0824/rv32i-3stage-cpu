...
module tb_Branch_Unit;
    ...
    initial begin
        ...
        check_branch(1'b1, 1'b0, `F3_BRANCH_NONE, 32'd0, 32'd0,
                     32'h0000_0500, 32'h0000_0008,
                     1'b0, 32'h0000_0508, "reserved branch operation 010 is safe");
        check_branch(1'b1, 1'b0, 3'b011, 32'd0, 32'd0,
                     32'hFFFF_FFFC, 32'h0000_0008,
                     1'b0, 32'h0000_0004, "reserved operation and target wraparound");

        // JAL must be unconditional.  Use equal operands with F3_JAL (001),
        // which overlaps BNE, so this cannot accidentally pass via comparison.
        check_branch(1'b1, 1'b1, `F3_JAL, 32'd9, 32'd9,
                     32'h0000_0100, 32'h0000_0014,
                     1'b1, 32'h0000_0114, "JAL takes positive PC-relative jump unconditionally");
        check_branch(1'b1, 1'b1, `F3_JAL, 32'd3, 32'd3,
                     32'h0000_0100, 32'hFFFF_FFF0,
                     1'b1, 32'h0000_00F0, "JAL takes negative PC-relative jump unconditionally");
        check_branch(1'b0, 1'b1, `F3_JAL, 32'd1, 32'd2,
                     32'h0000_0200, 32'h0000_0008,
                     1'b0, 32'h0000_0208, "branch_en gates jump request");

        // JALR target is (rs1 + imm) with bit 0 forced to zero.  PC is
        // deliberately unrelated so a mistaken PC-relative target will fail.
        check_branch(1'b1, 1'b1, `F3_JALR, 32'h0000_0120, 32'hDEAD_BEEF,
                     32'h0000_0500, 32'h0000_0015,
                     1'b1, 32'h0000_0134, "JALR uses rs1 base and clears target bit zero");
        check_branch(1'b1, 1'b1, `F3_JALR, 32'h0000_0110, 32'hCAFE_BABE,
                     32'h0000_0600, 32'hFFFF_FFEF,
                     1'b1, 32'h0000_00FE, "JALR sign-extended negative immediate target");
        check_branch(1'b0, 1'b1, `F3_JALR, 32'h0000_0120, 32'h0000_0000,
                     32'h0000_0700, 32'h0000_0015,
                     1'b0, 32'h0000_0134, "branch_en gates JALR redirect");
